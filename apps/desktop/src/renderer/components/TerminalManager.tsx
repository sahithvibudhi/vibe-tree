...
export function TerminalManager({ worktreePath, projectId, theme }: TerminalManagerProps) {
  const [worktreeTerminals, setWorktreeTerminals] =
    useState<Map<string, WorktreeTerminals>>(worktreeTerminalsCache);
  ...
  return (
    <div ref={containerRef} className="terminal-manager-root flex-1 h-full relative">
      ...
      <div
        className={`${currentSplitDirection === 'horizontal' ? 'flex flex-col' : 'flex'} h-full overflow-hidden`}
      >
            ...
            style={
              currentSplitDirection === 'vertical'
                ? {
                    ...
                    height: '100%',
                    borderRight:
                      index < currentTerminals.length - 1 ? '1px solid var(--border)' : 'none'
                  }
                : {
                    width: '100%',
