...
test.describe('Terminal Split Close Retry', () => {
  ...
  test.skip('should allow closing terminal split', async () => {
     ...
     * The test involves opening a terminal (PTY process) and splitting/closing terminals, which causes the
     * Electron app to become unresponsive during teardown in the CI environment. Failed 3 times (9+ minutes
     * total) at line 105 in afterEach hook trying to close the Electron app.
  ...
  test.skip('should allow multiple rapid close attempts on different terminals', async () => {
    /**
     * TODO: This test consistently hangs in CI after 2.6 minutes per attempt, causing worker teardown to timeout.
     * The test involves opening a terminal (PTY process) and splitting terminals multiple times, which causes
     * the Electron app to become unresponsive during teardown in the CI environment. Failed 3 times (7.8+ minutes
     * total) at line 174 trying to execute the test, hanging during worker teardown.
     * See errors/scheduler-tests-hang-in-ci.md for detailed analysis.
     */
    test.setTimeout(60000);
  ...
  test.skip('should display detailed backtrace when terminal close fails', async () => {
     ...
     * Failed 3 times (9+ minutes total) at line 229 trying to execute the test, hanging during worker teardown.
     * See errors/scheduler-tests-hang-in-ci.md for detailed analysis.
     */
    test.setTimeout(60000);
