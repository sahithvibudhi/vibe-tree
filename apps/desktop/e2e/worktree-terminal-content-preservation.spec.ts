...
test.describe('Worktree Terminal Content Preservation', () => {
  ...
  test('should preserve terminal content when switching between worktrees', async () => {
    ...
    const mainButton = page.locator(
      'button[data-worktree-branch="main"], button[data-worktree-branch="master"]'
    );
    ...
    console.log(
      `Main terminal content preview: ${mainTerminalContentBefore?.substring(0, 200)}...`
    );
    ...
    console.log(
      `Main terminal content after switching back: ${mainTerminalContentAfter?.substring(0, 200)}...`
    );
